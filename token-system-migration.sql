...
-- Add token-related tables to support credit-based billing
...
CREATE TABLE IF NOT EXISTS token_purchases (
  ...
  package_id VARCHAR(50) NOT NULL, -- starter, basic, premium, etc.
  cubes_purchased INTEGER NOT NULL,
  price_usd DECIMAL(10,2) NOT NULL,
  currency VARCHAR(10) NOT NULL, -- BTC, ETH, etc.
  payment_id VARCHAR(255), -- nowpayments transaction ID
  payment_status VARCHAR(50) DEFAULT 'pending', -- pending, completed, failed, expired
  nowpayments_data JSONB, -- store full nowpayments response
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  completed_at TIMESTAMP WITH TIME ZONE,
  
  -- Indexes
  INDEX idx_token_purchases_user_id (user_id),
  INDEX idx_token_purchases_payment_id (payment_id),
  INDEX idx_token_purchases_status (payment_status)
);
...
CREATE TABLE IF NOT EXISTS token_transactions (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  action_type VARCHAR(50) NOT NULL, -- 'email_fetch', 'connection_test'
  ...
  INDEX idx_token_transactions_action_type (action_type),
  INDEX idx_token_transactions_sync_job_id (sync_job_id),
...
-- Initialize existing users with 0 token balance
